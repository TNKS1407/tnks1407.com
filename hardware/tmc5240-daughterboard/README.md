...
- **θJA 実値（自己レビューで確認）: TQFN32 = 29 ℃/W（4層 JEDEC 基板）/ 47 ℃/W（1層）、θJC 1.7 ℃/W**。
  - Rev.2a（60×42, 4層）: JEDEC 基板よりやや小さいので実効 30〜38 ℃/W → ΔT ≈ 40〜55 ℃、室温で Tj ≈ 75〜80 ℃ ✓
  - Rev.2b（50×36, 4層）: 実効 ~40〜47 ℃/W → ΔT ≈ 55〜65 ℃。1.5A RMS 連続は成立圏内だが余白薄
- いずれも **EP 直下サーマルビア9本＋GND ベタ**を実装済み。停止時は IHOLD=10 で電流を落とす。
...
## 5.6 Rev.2b コンパクト版（50×36mm、`kicad/generate_pcb_compact.py`）

Rev.2a（60×42）の**トポロジ・回路・J1/J3 ピン配列をそのまま**に、面積を −50% した派生版。**KiCad 7 DRC エラー 0・未接続 0**。

- **サイズの意味**: JLCPCB の**4層最安ティア（50×50mm 以内）に収まる**。60mm 幅の Rev.2a はティア超えだったため、小型化がそのままコストダウンになる
- **全配線 45° 面取り**（`track()` ヘルパーが直角コーナーを自動チャンファ）
- 変更点: D1(TVS) は J3 上の空き地へ移動（SMBJ33A のまま）、マウント穴は M2×2（右下・左下）、C10–13 相当は引き続き非実装
- **放熱トレードオフ**: 銅面積半減で θJA ≈ 45 K/W 見込み（Rev.2a 比 +10）。1.5A RMS 連続は成立圏内だが余白減。**IHOLD 低減運用を推奨**、連続高負荷なら Rev.2a を選ぶ
- 製造データ: `kicad/fab-compact/`（ガーバー zip / BOM / CPL）。どちらを発注するかで `fab/`（Rev.2a）と使い分け
...
- [ ] TVS クランプ電圧が VS 絶対最大（38V）を超える条件を理解しているか（§8 参照。大回生用途は OV クランプ追加）／Q1 耐圧 −40V 以上か
- [ ] EP サーマルビア・GND ベタ・IHOLD 低減の放熱3点セット
- [ ] 発注直前の在庫・Basic/Extended 再確認（特に U1）

## 8. 自己レビュー結果（2026-07-02 実施）と残る未確認事項

データシート PDF 原本は取得経路が全て遮断されたため、**到達可能な一次情報での機械照合レビュー**を実施した。使用ソース: ADI 公式レジスタ定義（TMC-API `TMC5240_HW_Abstraction.h`）、Klipper 公式ドライバ（`tmc2240.py`、同一電流アーキテクチャ）、独立した実設計3ソースのシンボル/回路図（RPZ-Stepper 系 .kicad_sym、equilibot、MIT MediaLab pan-tilt）。

### 検証済み（機械照合で確認）

| 項目 | 結果 |
|---|---|
| U1 ピン配置（33ピン） | **3つの独立ソースと全一致**（スクリプトで pin番号→名称を突き合わせ） |
| 電流設定式 | Klipper 実装と完全一致: `KIFS=[11.75,24,36]A·kΩ`, `I_RMSmax=KIFS/Rref/√2`, `GS=round(256·I/I_RMSmax)` → **12kΩ+3Aレンジ+GS=181 で 1.500A RMS（丸めまで一致）**。Rref=12kΩ は Klipper の既定値でもある |
| GLOBALSCALER 制約 | 公式ヘッダ記載と一致（1–31 禁止、>128 推奨、0=フルスケール） |
| 標準回路定数 | 22nF(CPO–CPI)・2.2µF(VDD1V8)・1µF(VCP)・IREF 12k/13k を実設計2件で確認 |
| **θJA/θJC 実値** | TQFN32: **29 ℃/W（4層 JEDEC 基板）/ 47 ℃/W（1層）、θJC 1.7 ℃/W**。§4 の見積（35〜45）は安全側で妥当 |
| VS 絶対最大 | **38 V**（絶対最大表の断片「CPO ≤ min(44, VS+6)V」の44上限から逆算、姉妹品 TMC2240=38V とも一致。直接記載の確認は未） |
| Rev.2a ⇔ Rev.2b | 全112パッドのネット割当を diff、**電気的に同一**（差分は H3 削除と R2 のパッド順のみ＝抵抗なので無影響） |
| Q1 ピン配置 | SOT-23 単体 P-MOSFET は業界横断で 1=G/2=S/3=D（ST/onsemi/MCC で確認）。基板はこの通り |

### レビューで見つかった問題（正直に）

**⚠ TVS（SMBJ33A）は高エネルギーサージに対して完全ではない。** VS 絶対最大 38V に対し、SMBJ33A のクランプ電圧は定格サージ電流時 **最大 53.3V** — ESD や小サージには有効だが、大きな誘導性サージ／モータ回生では 38V を超えるおそれがある。スタンドオフを下げても（SMBJ30A 等）クランプは 38V を下回らないため、**TVS 単体では構造的に解決しない**（36V級ドライバ＋24V系の一般的な妥協点）。データシートが想定する本来の対策は **OV ピン＋外付け N-FET クランプ**（MIT MediaLab 設計は実装済み、本設計は未実装・OV開放）。

**運用条件**: 安定化24V電源・回生エネルギー小（通常のNEMA17位置決め用途）なら現状で実用可。長い減速で大きな回生が出る用途・電源が不安定な環境では **Rev.3 で OV クランプ回路（N-FET+抵抗2本+ショットキー）の追加を推奨**。

### 残る未確認（PDF 原本が読めたら潰す）

1. VS 絶対最大 38V の直接記載確認（傍証は強い）
2. R_IREF 許容帯の正式値（12–60k か 12–24k か。12kΩ はどちらでも適合）
3. S-LP2408LT1G 固有データシートのピン配置（業界慣例とは一致）
4. C10–13 用 C1588 の Basic 区分
