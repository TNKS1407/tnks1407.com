...
管の中に定在波を作るとき、**両端の条件**が効いてくる。

- **開いた端**は空気が自由に出入りできる → 空気が大きく動ける → **腹**になる。
- **閉じた端**は空気が動けない（壁にふさがれている）→ **節**になる。
...
ここで $c$ は音速（約 $340\,\mathrm{m/s}$）。片側開・片側閉の管は、いちばん低い共鳴で「管の長さ＝4分の1波長」になるのがポイントだ。

> ためしに $L=17\,\mathrm{cm}$、片側開・片側閉でやってみると、$f_1 = c/4L = 34000/(4\times17) = 500\,\mathrm{Hz}$。次は $1500\,\mathrm{Hz}$、その次は $2500\,\mathrm{Hz}$ ── と奇数倍に並ぶ。この $17\,\mathrm{cm}$ という長さ、覚えておいて。
...
そして、その素の音が **声道** ── 声帯から唇までの、長さおよそ $17\,\mathrm{cm}$ の管 ── を通る。声道は、声帯側がほぼ閉、唇側が開。さっきの「片側開・片側閉の管」そのものだ。だから素の音のうち、$500$、$1500$、$2500\,\mathrm{Hz}$ あたりが選ばれて強く響く。この **強く響く周波数の山** を、低いほうから **フォルマント** $F_1, F_2, F_3$ と呼ぶ。

声道の共鳴は、**圧力**で見るのが自然だ（マイクが拾うのも耳が感じるのも、空気のずれそのものより圧力のほうだからね）。下は同じ $17\,\mathrm{cm}$ の管を圧力で描いたもの。声帯側（閉じた端）が圧力の腹、唇側（開いた端）が圧力の節になっている ── さっきの「腹と節の入れ替わり」がそのまま効いている。
...
母音の違いは、ここで決まる。**舌や顎を動かすと、声道という管の「太さ」が場所ごとに変わる**。一様な太さの管なら $500/1500/2500$ ぴったりだけれど、どこかにくびれを作ると、その位置に応じて $F_1$ や $F_2$ が上下にずれる。
...
- **管** は、両端の条件（開＝腹、閉＝節）に「ぴったり収まる」波長だけを **共鳴** させる。$17\,\mathrm{cm}$ の片閉管なら $500/1500/2500\,\mathrm{Hz}$。
