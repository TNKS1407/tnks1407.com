...
> **積分を使わない見方も。** 力が一定の場合に限れば、高校で習う等加速度の式 $v_1^{\,2} = v_0^{\,2} + 2a\,d$（距離 $d$ を進むあいだに、速さの2乗がこれだけ増える）がそのまま使える。両辺に $\tfrac12 m$ を掛けると $\tfrac12 m v_1^{\,2} = \tfrac12 m v_0^{\,2} + (ma)\,d$。$ma$ は力 $F$ だから、移項すれば $F\,d = \tfrac12 m v_1^{\,2} - \tfrac12 m v_0^{\,2}$ ── 積分を一度も使わずに、同じ「された仕事＝運動エネルギーの変化」が出てくる。さっきの積分版は、力が途中で変わってもいいように、この一定力の式を細かく刻んで足し合わせた一般版だ、と思えばいい。同じ結論に、微分積分の道と、中学高校の運動の式の道、二つの入口から着く。
...
ここで、エネルギーという量の “タイプ” を、はっきりさせておくと見通しがいい。物理量には、性格の違う二種類がある。

- **状態量** ── その瞬間の状態（配置）だけで決まり、どんな道を通ってそこへ来たかによらない量。位置エネルギー $U$、運動エネルギー $K$、運動量、電荷、コップの水の量…。「いま、どれだけ持っているか」を表す。
- **流量** ── 状態量を、ある状態から別の状態へ **移す** 量。その移動量・変化量だ。仕事、力積…。「どれだけ動かしたか」を表す。

この目で見ると、さっきまでの話が一語で片づく。**仕事は流量、エネルギーはその状態量**だ。仕事（流量）をした分だけ、エネルギー（状態量）が移る。位置エネルギーが「経路によらない」のも当たり前で ── 状態量とは、そもそも経路によらない量のことだったから。保存力とは、その仕事から状態量（位置エネルギー）をちゃんと定義できる力のこと、と言い換えてもいい。

同じ対が、力学のもう一本の柱にもある。**力積は流量、運動量はその状態量**だ（力積を受けた分だけ、運動量が移る）。── 物理量を「状態量か、流量か」で分けて眺めるクセをつけると、力学だけでなく、電気でも熱でも「何が溜まる量で、何が移す量か」がすっと見えてくる。[⑥ 発散とガウスの法則](/docs/divergence-and-gauss/) で数えたフラックス（面を貫いて流れ出ていく量）も、この “流量” の親戚だよ。
...
ひとつ、正直に置いておきたいことがある。エネルギーは、「仕事をした分だけ変わる量」として、こちらが **定義した** ものだ。だから “保存するように作った量” という言い方も、半分は当たっている。── でも、それを「ただの便宜」と切り捨てるのは早い。その同じ定義が、力学でも電気でも熱でも、判で押したように帳尻を合わせてくれる。これだけ広く・しつこく効くということ自体が、エネルギーが単なる思いつきではなく、世界の確かな性質を捉えている証なんだ。じつは「存在する」って、突きつめると「保存する」とほとんど同じことなのかもしれない ── この見方は、いつか別の回でゆっくり追ってみよう。
