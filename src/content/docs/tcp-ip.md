...
- **スイッチ**は、いちばん外の **Ethernet（宛先 MAC）だけ**を見る。「この MAC はどの口か」を見て、その口へ流す ── それだけ。中の IP も TCP も**開けない**。だから単純で速い。同じご近所の中で配るだけの係だ。
- **ルータ**は、その一枚内側の **IP まで**開ける。宛先 IP から「次はどっちへ渡すか」を決める。そして大事な一手 ── ルータは**外の Ethernet(MAC) を、次の相手用に貼り替える**。でも中の IP・TCP・データには触らない。つまり **MAC は“隣へ”の使い捨て（ホップごとに貼り替わる）、IP は“最終目的地”まで不変（エンド・ツー・エンド）**。
- **相手のサーバ**で、ようやく **TCP・データまで**全部むかれる。芯のデータを開けたのは、結局、送り主と受け手の二人だけ、ということになる。
...
<div class="demo-embed">
  <iframe src="/packet-journey/" title="パケットの旅 ── 各機器はどこまで開けるか" loading="lazy"></iframe>
  <div class="cap"><span>packet-journey</span><a href="/packet-journey/" target="_blank" rel="noopener noreferrer">全画面で開く ↗</a></div>
</div>
